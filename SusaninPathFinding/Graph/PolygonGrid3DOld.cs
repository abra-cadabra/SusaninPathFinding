...
namespace Tektosyne.Geometry {
    ...
    /// cref="IGraph2D{T}"/> interface. The graph nodes are the <see cref="Point3D"/> coordinates of
    ...
    [Serializable]
    public class PolygonGrid3DOld : ICloneable, IGraph2D<Point3D>
    {
        ...
        private static readonly Point3D[][][] _squareEdgeOffsets = {
            new Point3D[][] {
                new Point3D[] {
        ...
        private static readonly Point3D[][][] _squareVertexColumnOffsets = {
            new Point3D[][] {
                new Point3D[] {
                    // edge only, column up
                    new Point3D( 1, -1),  // North-East
                    new Point3D( 1,  0),  // South-East
                    new Point3D(-1,  0),  // South-West
                    new Point3D(-1, -1),  // North-West
                },
                new Point3D[] {
                    // edge only, column down
                    new Point3D( 1,  0),  // North-East
                    new Point3D( 1,  1),  // South-East
                    new Point3D(-1,  1),  // South-West
                    new Point3D(-1,  0),  // North-West
                }
            },
            new Point3D[][] {
                new Point3D[] {
                    // edge & vertex, column up
                    new Point3D( 0, -1),  // North
                    new Point3D( 1, -1),  // North-East
                    new Point3D( 2,  0),  // East
                    new Point3D( 1,  0),  // South-East
                    new Point3D( 0,  1),  // South
                    new Point3D(-1,  0),  // South-West
                    new Point3D(-2,  0),  // West
                    new Point3D(-1, -1),  // North-West
                },
                new Point3D[] {
        ...
        private static readonly Point3D[][][] _squareVertexRowOffsets = {
            new Point3D[][] {
                new Point3D[] {
                    // edge only, row left
                    new Point3D( 0, -1),  // North-East
                    new Point3D( 0,  1),  // South-East
                    new Point3D(-1,  1),  // South-West
                    new Point3D(-1, -1),  // North-West
                },
                new Point3D[] {
                    ...
                    new Point3D( 1, -1),  // North-East
                    new Point3D( 1,  1),  // South-East
                    new Point3D( 0,  1),  // South-West
                    new Point3D( 0, -1),  // North-West
                }
            },
            new Point3D[][] {
                new Point3D[] {
                    // edge & vertex, row left
                    new Point3D( 0, -2),  // North
                    new Point3D( 0, -1),  // North-East
                    new Point3D( 1,  0),  // East
                    new Point3D( 0,  1),  // South-East
                    new Point3D( 0,  2),  // South
                    new Point3D(-1,  1),  // South-West
                    new Point3D(-1,  0),  // West
                    new Point3D(-1, -1),  // North-West
                },
                new Point3D[] {
        ...
        private static readonly Point3D[][] _hexagonEdgeOffsets = {
            new Point3D[] {
                // column up
                new Point3D( 0, -1),  // North
                new Point3D( 1, -1),  // North-East
        ...
        private static readonly Point3D[][] _hexagonVertexOffsets = {
            new Point3D[] {
                // row left
                new Point3D( 0, -1),  // North-East
                new Point3D( 1,  0),  // East
        ...
        /// <remarks><para>
        /// <b>InvalidLocation</b> holds a <see cref="Point3D"/> value whose <see cref="Point3D.X"/>
        /// and <see cref="Point3D.Y"/> components are both -1. This represents a location outside of
        ...
        public static readonly Point3D InvalidLocation = new Point3D(-1, -1);
        ...
        /// </summary>
        /// <value>
        /// A jagged <see cref="Array"/> containing 1 x 4, 2 x 4, or 2 x 6 <see cref="Point3D"/>
        ...
        public Point3D[][] EdgeNeighborOffsets {
            [DebuggerStepThrough]
        ...
        /// cref="Point3D"/> values whose coordinates range from -2 to +2.</value>
        /// <remarks><para>
        ...
        public Point3D[][] NeighborOffsets {
            [DebuggerStepThrough]
        ...
        private Point3D DisplayToGridCore(double x, double y) {
            ...
            if (Element.Sides == 4) {
                ...
                if (Element.Orientation == PolygonOrientation.OnEdge)
                    return new Point3D((int) (x / width), (int) (y / height));
                ...
                if (AreColumnsShifted) {
                    ...
                    if (Math.Abs(y) <= x + width2)
                        return new Point3D(column, row);
                    else {
                        Point3D offset = EdgeNeighborOffsets[index][y < 0 ? 3 : 2];
                        return new Point3D(column + offset.X, row + offset.Y);
                    }
                }
                else {
                    // determine closest element
                    ...
                    if (Math.Abs(x) <= y + height2)
                        ...
                        Point3D offset = EdgeNeighborOffsets[index][x < 0 ? 3 : 0];
                        return new Point3D(column + offset.X, row + offset.Y);
                    }
                }
            }
                ...
                if (AreColumnsShifted) {
                    ...
                    if (Math.Abs(y) <= (x + width2) * PolygonGrid._sqrt3)
                        ...
                        Point3D offset = EdgeNeighborOffsets[index][y < 0 ? 5 : 4];
                        return new Point3D(column + offset.X, row + offset.Y);
                    }
                }
                else {
                    ...
                    if (Math.Abs(x) <= (y + height2) * PolygonGrid._sqrt3)
                        ...
                        Point3D offset = EdgeNeighborOffsets[index][x < 0 ? 5 : 0];
                        return new Point3D(column + offset.X, row + offset.Y);
                    }
                }
            }
        ...
        public Point3D DisplayToGrid(double x, double y) {
            /*
            ...
            Point3D element = DisplayToGridCore(x, y);
        ...
        public Point3D DisplayToGrid(PointD location) {
        ...
        public Point3D DisplayToGridClipped(double x, double y) {
            /*
        ...
        public Point3D DisplayToGridClipped(PointD location) {
        ...
        public Point3D[] GetEdgeNeighborOffsets(Point3D location) {
        ...
        #endregion
        #region GetElementBounds(Point3D)

        /// <summary>
        /// Gets the bounding rectangle of the <see cref="Element"/> at the specified <see
        /// cref="Point3D"/> location.</summary>
        /// <param name="location">
        ...
        public RectD GetElementBounds(Point3D location) {
        ...
        public Point3D GetNeighbor(Point3D location, int index) {

            // offset location by normalized index in its group
            Point3D[] offsets = GetNeighborOffsets(location);
        ...
        public int GetNeighborIndex(Point3D location, Point3D neighbor) {

            // determine actual offset of specified neighbor
            Point3D offset = new Point3D(neighbor.X - location.X, neighbor.Y - location.Y);
        ...
        public Point3D[] GetNeighborOffsets(Point3D location) {
            int index = ((IsRightRow(location.Y) || IsDownColumn(location.X)) ? 1 : 0);
        ...
        public int GetStepDistance(Point3D source, Point3D target) {
        ...
        public PointD GridToDisplay(int column, int row) {
            Point3D factor; // to be multipled by (width/4, height/4)

            if (Element.Sides == 4) {
                if (Element.Orientation == PolygonOrientation.OnEdge)
                    factor = new Point3D(4 * column + 2, 4 * row + 2);
                else
                    factor = (AreColumnsShifted ?
                        new Point3D(2 * column + 2, 4 * row + (IsDownColumn(column) ? 4 : 2)) :
                        new Point3D(4 * column + (IsRightRow(row) ? 4 : 2), 2 * row + 2));
            }
            else {
                Debug.Assert(Element.Sides == 6);

                factor = (AreColumnsShifted ?
                    new Point3D(3 * column + 2, 4 * row + (IsDownColumn(column) ? 4 : 2)) :
                    new Point3D(4 * column + (IsRightRow(row) ? 4 : 2), 3 * row + 2));
            }
        ...
        public PointD GridToDisplay(Point3D location) {
        ...
        public SubdivisionMap ToSubdivision(PointD offset, double epsilon = 0) {
            ...
            var faceToGrid = new Point3D[width * height];
            ...
            for (int x = 0; x < width; x++)
                for (int y = 0; y < height; y++) {
                    ...
                    faceToGrid[face._key - 1] = new Point3D(x, y);
                    gridToFace[x, y] = face;
                }
        ...
        /// <remarks>
        /// <b>Nodes</b> returns all <see cref="Point3D"/> locations in the <see
        /// cref="PolygonGrid"/>, starting at (0,0) and incrementing x-coordinates before
        /// y-coordinates.</remarks>

        public IEnumerable<Point3D> Nodes {
            get {
        ...
        public bool Contains(Point3D node) {
        ...
        public double GetDistance(Point3D source, Point3D target) {
        ...
        public Point3D GetNearestNode(PointD location) {
            return DisplayToGridClipped(location.X, location.Y);
        }

        #endregion
        #region GetNeighbors(Point3D)
        ...
        /// cref="IList{Point3D}"/> if the specified <paramref name="node"/> or all its neighbors are
        ...
        public IList<Point3D> GetNeighbors(Point3D node)
        {

            // do nothing if specified location invalid
            if (!Contains(node)) return new List<Point3D>(0);

            // determine offset group for given location
            Point3D[] offsets = GetNeighborOffsets(node);
            List<Point3D> neighbors = new List<Point3D>(offsets.Length);

            // build list containing all valid neighbors
            for (int index = 0; index < offsets.Length; index++) {
                Point3D neighbor = node + offsets[index];
        ...
        #endregion
        #region GetNeighbors(Point3D, Int32)
        ...
        /// cref="IList{Point3D}"/> are ordered by increasing x- and y-coordinates. 
        /// </para><para>
        ...
        public IList<Point3D> GetNeighbors(Point3D node, int distance) {
            if (distance <= 0)
                ThrowHelper.ThrowArgumentOutOfRangeException(
            ...
            for (int x = minX; x <= maxX; x++)
                for (int y = minY; y <= maxY; y++) {
                    Point3D neighbor = new Point3D(x, y);
        ...
        public PointD GetWorldLocation(Point3D node) {
        ...
        public PointD[] GetWorldRegion(Point3D node) {
        ...
        private class InstanceData {
            ...
            internal Point3D[][] EdgeNeighborOffsets, NeighborOffsets;
        ...
        /// cref="Subdivision"/> and the <see cref="Point3D"/> locations of the <see
        ...
        [Serializable]
        public class SubdivisionMap: ISubdivisionMap<Point3D> {
            ...
            internal SubdivisionMap(Subdivision source, PolygonGrid target,
                Point3D[] faceToGrid, SubdivisionFace[,] gridToFace) {
            ...
            // mapping arrays
            private readonly Point3D[] _faceToGrid;
            ...
            object ISubdivisionMap<Point3D>.Target {
                get { return _target; }
            }
            ...
            public Point3D FromFace(SubdivisionFace face) {
            ...
            public SubdivisionFace ToFace(Point3D value) {
